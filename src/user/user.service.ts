...
export class UserService {
  constructor(@InjectModel(Collections.User) private readonly userModel: Model<User>) {}
  ...
  async createUser(data: { displayName: string; email: string; password: string }): Promise<UserDocument> {
    ...
    const dbUser = new this.userModel({
      ...
      validations: [{ emailVerification: uuidv4() }],
    });
